...
The gateway creates a unique 0700 directory beneath its temporary directory or `CODESPACE_RUNNER_DIR`. The internal protocol is CodeSpace JSON with a u32 length prefix, handshake version 6, request IDs, and process-exit events. It is not Codex App Server RPC. Same-connection replay is not reconnect recovery.
