...
| `WORKSPACE_BUSY` | 라이브 프로세스가 작업 공간을 점유함. 기다리거나 프로세스를 취소. 요청 소유 패치·exec는 서버가 직렬화함. 빠른 반복 재시도 피하기 |
